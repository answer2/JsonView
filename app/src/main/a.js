private void initData() throws Throwable {
    ...
    // 基本信息位置获取
    Sheet sheet = excelTool.getSheet(Integer.valueOf(sheet_textInput.getText().toString())); // 获取第一个工作表
...
public void output(View view) {
    try {
        if (!gradeList.isEmpty()) {
            ...
            // 将数据写入单元格
            List<GradeResponse.Data> dataList = gradeList; // 获取你的数据列表，假设为List<YourBean>
            /* Collections.sort(
            dataList,
            new Comparator<GradeResponse.Data>() {
                @Override
                public int compare(GradeResponse.Data data1, GradeResponse.Data data2) {
                    return Integer.compare(Integer.valueOf(data1.getZf()), Integer.valueOf(data2.getZf()));
                }
            });*/
